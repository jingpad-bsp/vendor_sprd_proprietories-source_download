...
static int try_to_connect_device(int uart_fd) {
  ...
  for (;;) {
    ...
    for (i= 0; i<retry_max; i++) {
      ret = write(uart_fd, &hand_shake, 3);
      if (ret < 0) {
        DOWNLOAD_LOGD("UART Send HandShake %s %d again\n", strerror(errno), ret);
        usleep(100*1000);
        continue;
      }
      break;
    ...
    for(i= 0; i<retry_max*2; i++) {
      ret = read(uart_fd, version_string, 1);
      if (ret != 1) {
        DOWNLOAD_LOGD("UART read %s %d, %d, data: 0x%x\n", strerror(errno), ret,
               i, version_string[0]);
        usleep(100*1000);
        continue;
      }
...
static void store_client_fd(WcnClient client_fds[], int fd, int type) {
  ...
  if (i == WCN_MAX_CLIENT_NUM) {
    ...
    close(client_fds[i - 1].sockfd);
    DOWNLOAD_LOGD("%s: close client_fds[%d].sockfd=%d, because of FULL", __FUNCTION__, i-1, client_fds[i - 1].sockfd);
  ...
  while (1) {
    ...
    for (i = 0; i < WCN_MAX_CLIENT_NUM; i++) 
    {
      if ((pmanager->client_fds[i].sockfd >= 0) && 
	     ...
	     (pmanager->client_fds[i].type == WCN_SOCKET_TYPE_SLOG))) 
      {
    ...
    if (FD_ISSET(pmanager->selfcmd_sockets[1], &readset)) {
      ...
      if (ret) {
	DOWNLOAD_LOGD("wcn_exception_listen read pmanager->selfcmd_sockets[1]=%d %d bytes", pmanager->selfcmd_sockets[1],
                    ret);
	}
    ...
    for (i = 0; i < WCN_MAX_CLIENT_NUM; i++) {
      if ((pmanager->client_fds[i].sockfd >= 0) &&
        ...
        if ((pmanager->client_fds[i].type == WCN_SOCKET_TYPE_GNSS) || (pmanager->client_fds[i].type == WCN_SOCKET_TYPE_SLOG))
	{
		ret = read(pmanager->client_fds[i].sockfd, buffer, SOCKET_BUFFER_SIZE);
	if (ret) {
		DOWNLOAD_LOGD("wcn_exception_listen read pmanager->client_fds[%d].sockfd=%d, %d byte",
                        i, pmanager->client_fds[i].sockfd, ret);
	}
	   ...
	   {
	       DOWNLOAD_LOGD("wcn_exception_listen close pmanager->client_fds[%d].sockfd=%d here_harry", i, pmanager->client_fds[i].sockfd);
		close(pmanager->client_fds[i].sockfd);
		pmanager->client_fds[i].sockfd = -1;
	   }
...
static int gnss_get_image_path(void) {
  ...
  property_get("ro.vendor.ge2.partition", partition, NULL);
  property_get("ro.vendor.ge2.mode", ge2_img_mode, NULL);
  DOWNLOAD_LOGD("ro.vendor.ge2.partition:%s,ro.vendor.ge2.mode:%s", partition, ge2_img_mode);
