...
static unsigned char *uart_dev = UART_DEVICE_NAME;
...
static int download_images_count = 2;
...
void set_raw_data_speed(int fd, int speed) {
  ...
  for (i = 0; i < sizeof(speed_arr) / sizeof(int); i++) {
    if (speed == name_arr[i]) {
      // set raw data mode
      // Opt.c_iflag &= ~(IGNBRK | BRKINT | PARMRK | ISTRIP | INLCR | IGNCR |
      // ICRNL);
      Opt.c_oflag &= ~OPOST;
      ...
      if (version_string[0] == 0x7E) {
        ...
        do {
          ...
          if (ret == 1) {
            ...
            data++;
            if ((unsigned int)(data - version_string) >= sizeof(buffer)) {
...
#define MARLIN_AA_CHIP 0x2331FF
#define MARLIN_BA_CHIP_MIN 0x23310FFF
#define MARLIN_BA_CHIP_MAX 0x233110FF

typedef unsigned int uint32;
struct head {
  unsigned char magic[4];     // "WCNM"
  uint32 version;    // 0
  uint32 img_count;  // image count
};

struct imageinfo {
  unsigned char tag[4];    // "MLAA","MLBA"
...
struct img_flag {
  ...
  int which_img_flag;   // 1: is the one image,2: is the second image, just
                        // is_combine_flag = 1 is valid.
};
...
other: not ok
******************************************************/
int parse_image_info(int image_fd, unsigned char *strc) {
...
int parse_wcnimg_info(struct image_info *info, int chiptype) {
  ...
  if (read_len > 0) {
    if (read_len == packet_size) {
      ...
      if (ret == 0) {
        ...
        step = 2;
        // marlin_img_flag.is_combine_flag = 1;//just the combine image info is
        // right,it will be set this flag
      }
    }
  }
  // (step2): if yes,judge which image we need to parse(judge chip id)
  if (step == 2) {
    ...
    marlin_img_flag.is_combine_flag =
        1;  // just the combine image info is right,it will be set this flag
    step = 3;
  }
  // (step3): parse the image
...
int download_image(int channel_fd, struct image_info *info) {
  ...
  if (info->image_path == NULL) return DL_SUCCESS;
  if (info->image_size < HS_PACKET_SIZE) trans_size = LS_PACKET_SIZE;
  ...
  } else if ((MARLIN_BA_CHIP_MIN < chip_id) && (MARLIN_BA_CHIP_MAX > chip_id)) {
  ...
  if (marlin_img_flag.is_combine_flag == 1) {
    DOWNLOAD_LOGD("marlin_imginfo.offset = %d,AA=0x%x,BA=0x%x\n",
                  marlin_imginfo.offset, BSL_REP_IS_2331_AA,
                  BSL_REP_IS_2331_BA);
    if ((marlin_img_flag.which_img_flag == BSL_REP_IS_2331_AA) ||
      ...
      DOWNLOAD_LOGD(
          "the image is combine ,but is not the AA or BA !!!exit!!\n");
      close(image_fd);
      return -1;
    }
  }
  ...
  DOWNLOAD_LOGD(
      "=====image_size = %d, count=%d,trans_size=%d address=%d =====\n",
  ...
  for (i = 0; i < count; i++) {
    ...
    buffer = (unsigned char *)&test_buffer[8];
    do {
    ...
    if (image_size < trans_size) {
      for (i = image_size; i < trans_size; i++) test_buffer[i + 8] = 0xFF;
      image_size = 0;
    } else {
      image_size -= trans_size;
    }

    ret = send_data_message(channel_fd, test_buffer, trans_size, 1, trans_size,
                            image_fd);
    if (ret != DL_SUCCESS) {
      close(image_fd);
      return DL_FAILURE;
...
int download_images(int channel_fd) {
  ...
  for (i = 0; i < image_count; i++) {
    ...
    if (ret != DL_SUCCESS) return DL_FAILURE;
    info++;
  }
...
static void download_wifi_calibration(int download_fd) {
  ...
  ret = write(download_fd, &wifi_data.wifi_rf_cali,
              sizeof(wifi_data.wifi_rf_cali));
#ifndef WCN_EXTEN_15C
  do {
    ret = read(download_fd, &wifi_data.wifi_cali_cp,
               sizeof(wifi_data.wifi_cali_cp));
    usleep(1000);
    retry_cnt++;
  } while ((ret <= 0) && (retry_cnt <= 2000));

  if (ret <= 0) DOWNLOAD_LOGD("wait cali fail!!!!\n");
...
static int send_msg_to_mdbg(unsigned char *str) {
  int loop_fd = -1;
  int ret = 0;
  ...
  if (ret < 0) {
    DOWNLOAD_LOGE("write %s failed, error:%s loop_fd:%d", LOOP_DEV,
                  strerror(errno), loop_fd);
    close(loop_fd);
    return -1;
  }
  close(loop_fd);
...
int download_entry(void) {
  ...
  int download_fd = -1;
  int ret = 0;
  unsigned char value[PROPERTY_VALUE_MAX] = {'\0'};
  bool reboot = false;
  ...
  if (pmanager.flag_stop) {
    ...
    download_power_on(0);
#endif
    ret = send_notify_to_client(&pmanager, WCN_RESP_STOP_WCN,
                                WCN_SOCKET_TYPE_WCND);
  ...
  if (pmanager.flag_start) {
    ...
    ret = send_notify_to_client(&pmanager, WCN_RESP_START_WCN,
                                WCN_SOCKET_TYPE_WCND);
...
reboot_device:
  if (reboot) {
    ...
    DOWNLOAD_LOGE("failed to power up marlin,%d retry left\n", retry);
    usleep(3000);
    reboot = false;
  }
  download_power_on(1);
  ...
  if (pmanager.flag_dump) {
    ...
    ret = send_notify_to_client(&pmanager, WCN_CMD_START_DUMP_WCN,
  ...
  if (pmanager.flag_dump) {
    ret = send_notify_to_client(&pmanager, WCN_RESP_DUMP_WCN,
                                WCN_SOCKET_TYPE_WCND);
    pmanager.flag_dump = 0;
  }

  if (pmanager.flag_reboot) {
    ret = send_notify_to_client(&pmanager, WCN_RESP_REBOOT_WCN,
                                WCN_SOCKET_TYPE_WCND);
...
static void *client_listen_thread(void *arg) {
  ...
  while (1) {
    ...
    if (FD_ISSET(pmanager->listen_slog_fd, &read_fds)) {
      ...
      if (c < 0) {
        DOWNLOAD_LOGE("accept %s failed (%s)", WCN_SOCKET_SLOG_NAME,
                      strerror(errno));
        sleep(1);
        continue;
      }
      ...
      if (c < 0) {
        DOWNLOAD_LOGE("accept %s failed (%s)", WCN_SOCKET_NAME,
                      strerror(errno));
        sleep(1);
        continue;
      }
  ...
  while (1) {
    ...
    for (i = 0; i < WCN_MAX_CLIENT_NUM; i++) {
      if ((pmanager->client_fds[i].sockfd >= 0) &&
          (pmanager->client_fds[i].type == WCN_SOCKET_TYPE_WCND)) {
...
static int socket_init(pmanager_t *pmanager) {
  ...
  pmanager->listen_fd = socket_local_server(
      WCN_SOCKET_NAME, ANDROID_SOCKET_NAMESPACE_ABSTRACT, SOCK_STREAM);
  ...
  pmanager->listen_slog_fd = socket_local_server(
      WCN_SOCKET_SLOG_NAME, ANDROID_SOCKET_NAMESPACE_ABSTRACT, SOCK_STREAM);
